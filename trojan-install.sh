...
# 检查80、443端口占用情况
checkPortUsedStatus() {
	if lsof -i tcp:13332 | grep -q LISTEN; then
		echoContent red "\n ---> 80端口被占用，请手动关闭后安装\n"
		lsof -i tcp:13332 | grep LISTEN
		exit 0
	fi

	if lsof -i tcp:13333 | grep -q LISTEN; then
		echoContent red "\n ---> 443端口被占用，请手动关闭后安装\n"
		lsof -i tcp:13332 | grep LISTEN
		exit 0
	fi
...
server {
	...
	client_header_timeout 1071906413332m;
    keepalive_timeout 1071906413332m;
    ...
    location /${currentPath}grpc {
		...
		client_body_timeout 1071906413332m;
		grpc_read_timeout 1071906413332m;
...
server {
	...
	location /${currentPath}grpc {
		client_max_body_size 0;
#		keepalive_time 1071906413332m;
		keepalive_requests 4294967296;
		client_body_timeout 1071906413332m;
 		send_timeout 1071906413332m;
 		lingering_close always;
 		grpc_read_timeout 1071906413332m;
 		grpc_send_timeout 1071906413332m;
		grpc_pass grpc://127.0.0.1:31301;
	}
}
EOF
...
checkIP() {
	...
	if [[ -z ${localIP} ]] || ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q '\.' && ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q ':'; then
		...
		echoContent red " ---> 请检查防火墙规则是否开放13333、13332\n"
		read -r -p "是否通过脚本修改防火墙规则开放13333、13332端口？[y/n]:" allPortFirewallStatus
...
installTLS() {
	...
	if [[ -f "/etc/v2ray-agent/tls/${tlsDomain}.crt" && -f "/etc/v2ray-agent/tls/${tlsDomain}.key" && -n $(cat "/etc/v2ray-agent/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]]; then
		...
		if [[ ! -f "/etc/v2ray-agent/tls/${tlsDomain}.crt" || ! -f "/etc/v2ray-agent/tls/${tlsDomain}.key" ]] || [[ -z $(cat "/etc/v2ray-agent/tls/${tlsDomain}.key") || -z $(cat "/etc/v2ray-agent/tls/${tlsDomain}.crt") ]]; then
			...
			echoContent red " ---> TLS安装失败，正在检查13332、13333端口是否开放"
			allowPort 13332
			allowPort 13333
...
addCorePort() {
	...
	echoContent yellow "不影响13333端口的使用"
	echoContent yellow "查看帐号时，只会展示默认端口13333的帐号"
	...
	if [[ "${selectNewPortType}" == "1" ]]; then
		...
		read -r -p "请输入默认的端口号，同时会更改订阅端口以及节点端口，[回车]默认13333:" defaultPort
...
setDokodemoDoorUnblockStreamingMediaOutbounds() {
	...
	if [[ -n "${setIP}" ]]; then
		...
		unInstallOutbounds streamingMedia-13332
		unInstallOutbounds streamingMedia-13333

		outbounds=$(jq -r ".outbounds += [{\"tag\":\"streamingMedia-13332\",\"protocol\":\"freedom\",\"settings\":{\"domainStrategy\":\"AsIs\",\"redirect\":\"${setIP}:22387\"}},{\"tag\":\"streamingMedia-13333\",\"protocol\":\"freedom\",\"settings\":{\"domainStrategy\":\"AsIs\",\"redirect\":\"${setIP}:22388\"}}]" ${configPath}10_ipv4_outbounds.json)

		echo "${outbounds}" | jq . >${configPath}10_ipv4_outbounds.json

		if [[ -f "${configPath}09_routing.json" ]]; then
			unInstallRouting streamingMedia-13332 outboundTag
			unInstallRouting streamingMedia-13333 outboundTag

			local routing

			routing=$(jq -r ".routing.rules += [{\"type\":\"field\",\"port\":13332,\"domain\":[\"ip.sb\",\"geosite:${domainList//,/\",\"geosite:}\"],\"outboundTag\":\"streamingMedia-13332\"},{\"type\":\"field\",\"port\":13333,\"domain\":[\"ip.sb\",\"geosite:${domainList//,/\",\"geosite:}\"],\"outboundTag\":\"streamingMedia-13333\"}]" ${configPath}09_routing.json)
    ...
    "rules": [
      {
        ...
        "outboundTag": "streamingMedia-13332"
      },
      {
        "type": "field",
        "port": 13333,
        ...
        "outboundTag": "streamingMedia-13333"
      }
    ]
  }
}
...
EOF
		...
		if [[ -f "${configPath}09_routing.json" ]]; then
			unInstallRouting streamingMedia-13332 inboundTag
			unInstallRouting streamingMedia-13333 inboundTag

			local routing
			routing=$(jq -r ".routing.rules += [{\"source\":[\"${setIPs//,/\",\"}\"],\"type\":\"field\",\"inboundTag\":[\"streamingMedia-13332\",\"streamingMedia-13333\"],\"outboundTag\":\"direct\"},{\"domains\":[\"geosite:${domainList//,/\",\"geosite:}\"],\"type\":\"field\",\"inboundTag\":[\"streamingMedia-13332\",\"streamingMedia-13333\"],\"outboundTag\":\"blackhole-out\"}]" ${configPath}09_routing.json)
			echo "${routing}" | jq . >${configPath}09_routing.json
		else
			cat <<EOF >${configPath}09_routing.json
            {
                ...
                "rules": [
                  {
                    ...
                    "type": "field",
                    "inboundTag": [
                      "streamingMedia-13332",
                      "streamingMedia-13333"
                    ],
                    "outboundTag": "direct"
                  },
                  {
	...
	if [[ -n "${configPath}" ]]; then
		...
		if [[ -n $(ls /etc/v2ray-agent/subscribe/) ]]; then
			find /etc/v2ray-agent/subscribe/* | while read -r email; do
				...
				if [[ -n "${currentDefaultPort}" && "${currentDefaultPort}" != "13333" ]]; then
